...
> **구현 상태: 설계 확정, 아직 미구현.** 현재 코드는 참여자별 선호를 하나의 목록으로 평탄화한 뒤 LLM이 후보 순서를 직접 결정한다. 아래 내용은 Back 요청·응답 계약을 바꾸지 않고 AI 내부에 추가할 계산 방식이다. 실제 구현과 테스트가 끝나기 전에는 발표에서 “구현했다”거나 “계산 결과를 검증했다”고 표현하지 않는다.

### 발표에서 먼저 말할 한 문장

> 참여자별 만족도를 따로 계산한 뒤 평균 만족도 70%와 최저 만족도 30%를 결합하고, 알레르기와 강한 비선호는 다른 사람의 높은 점수로 상쇄되지 않도록 후보 단계에서 강제 제외한다.
...
하지만 현재 AI 라우트는 각 참여자의 `preferences[]`를 하나의 `participant_preferences` 배열로 합친다.
...
따라서 공정성 점수의 첫 단계는 새로운 Back 필드를 추가하는 것이 아니라, **이미 받은 참여자 구분을 AI 내부에서 버리지 않는 것**이다.
...
| **평균 만족도 + 최저 만족도 + hard veto** | 전체 만족과 소외 방지를 함께 반영하고 안전 조건을 점수 경쟁에서 분리 | 가중치와 공정성 정의가 서비스 정책이며 실제 데이터로 조정이 필요 |

현재 선택은 마지막 방식이다. 전체 평균을 주축으로 두되 최저 만족도를 보완하고, 알레르기와 강한 비선호는 애초에 점수로 거래하지 않는다.
...
응답에도 `fairnessScore`나 `participantScores` 같은 새 필드를 추가하지 않는다. 계산 결과는 기존 `suggestions[].rank`, `reasons`, `tags`, `summary`를 결정하는 AI 내부 근거로만 사용한다. 따라서 Back과 Front는 수정하지 않아도 된다.
...
목록 밖 값은 버린다. LLM은 의미 관계만 판단하고, enum을 숫자로 변환하는 작업부터 점수 집계와 정렬까지는 서버 코드가 수행한다.
...
### 2.5 점수보다 먼저 적용하는 강제 제외

다음 조건은 낮은 점수를 주는 정도로 처리하지 않고 후보를 제거한다.

```text
Veto(c) = 어떤 참여자 i의 선호 p에 대해
          후보 c가 p와 충돌하고,
          p가 알레르기·안전 제약이거나
          (sentiment=NEGATIVE이고 strength=STRONG)
```

`Veto(c)`가 참이면 개인 만족도와 최종 점수를 계산하지 않는다.
...
참여자 B: KARAOKE / NEGATIVE / STRONG
후보: 노래방
→ 평균 점수 계산 전에 강제 제외
```

알레르기 여부는 자유 문자열이 아니라 AI 서버가 관리하는 안전 제약 Vocabulary 코드 집합으로 판별한다. 예를 들어 `SHELLFISH_ALLERGY`는 `SHELLFISH`, `CRAB`, `SHRIMP` 같은 위험 범주와 연결하는 내부 규칙이 필요하다.

넓은 비선호와 구체적인 긍정 예외가 같은 참여자 안에서 충돌하면 Vocabulary 계층의 구체성 규칙을 먼저 적용한다.

```text
같은 참여자:
SEAFOOD / NEGATIVE / STRONG
SHELLFISH / POSITIVE / MODERATE
후보: 조개 전문점
→ 더 구체적인 SHELLFISH 선호를 먼저 검토
```

다만 알레르기 같은 안전 제약은 구체적인 긍정 선호로 해제하지 않는다. 다른 참여자의 긍정 선호도 한 참여자의 hard veto를 상쇄할 수 없다.

### 2.6 개인별 만족도 계산

강제 제외를 통과한 후보에 대해서만 개인 만족도를 계산한다.
...
```text
Score(c) = 100 × [0.7 × A(c) + 0.3 × F(c)]
```

- `A(c)`의 70%는 그룹 전체의 만족도를 반영한다.
- `F(c)`의 30%는 가장 불리한 참여자를 보호한다.
- 최종 점수는 `0~100` 범위다.
- 알레르기와 강한 비선호는 계산보다 먼저 veto되므로 높은 평균으로 상쇄할 수 없다.

동점이면 다음 순서로 정렬한다.

1. 최저 만족도 `F(c)`가 높은 후보
2. 평균 만족도 `A(c)`가 높은 후보
...
| 후보 | 참여자 A | 참여자 B | 참여자 C | 평균 `A(c)` | 최저 `F(c)` | 최종 점수 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
| 후보 | 조건 | 처리 |
| --- | --- | --- |
| 조개구이 전문점 | 참여자 중 한 명이 갑각류 알레르기 | `VETO`, 점수 계산 안 함 |
| 노래방 | 참여자 중 한 명이 `KARAOKE / NEGATIVE / STRONG` | `VETO`, 점수 계산 안 함 |
...
flowchart LR
    ...
    C --> D{"알레르기 또는<br/>STRONG NEGATIVE 충돌?"}
    D -->|예| E["후보 강제 제외"]
    D -->|아니오| F["개인별 만족도 u(i,c) 계산"]
    F --> G["평균 A(c)·최저 F(c) 계산"]
    G --> H["0.7 × 평균 + 0.3 × 최저"]
    H --> I["점수순 정렬"]
...
| 알레르기·강한 비선호 veto | AI 서버 코드 |
| 개인 효용·평균·최솟값·최종 점수 | AI 서버 코드 |
| 점수순 후보의 추천 설명 문장 | LLM |
| 응답 저장·Front 전달 | 기존 Back 계약 |

LLM은 최종 후보 순위를 직접 결정하지 않는다. 서버가 점수로 순서를 고정한 뒤 LLM은 그 순서를 바꾸지 않고 집단 수준 추천 사유만 작성한다.
...
3. **안전 조건을 평균 점수로 상쇄하지 않는다.** 알레르기와 강한 비선호는 veto로 분리한다.
4. **결과가 재현 가능해진다.** 의미 판정 이후의 가중치, 평균, 최솟값, 정렬은 고정 코드로 계산한다.
5. **Back 계약을 바꾸지 않는다.** 기존 참여자 요청 구조와 기존 추천 응답을 그대로 사용한다.

### 2.12 현재 구현 상태와 구현 체크리스트

현재 구현 상태는 다음과 같다.

- `/candidates` 라우트가 참여자별 선호를 하나의 배열로 평탄화한다.
- Candidates graph 상태에는 참여자별 그룹 정보가 남지 않는다.
- Candidate Ranker가 LLM 응답 배열의 순서를 그대로 최종 순위로 사용한다.
- 개인 효용, 평균 만족도, 최저 만족도, 공정성 점수 계산이 없다.
- 알레르기와 `STRONG + NEGATIVE`를 결정론적으로 veto하는 코드가 없다.

실제 구현 시 필요한 작업:

1. 라우트에서 `participants[]` 구조를 그대로 그래프에 전달한다.
2. Candidates state가 참여자별 선호를 보존하도록 바꾼다.
3. 후보와 각 선호의 관계를 제한된 enum으로 판정하는 단계를 둔다.
4. 안전 제약 코드와 위험 Vocabulary 사이의 내부 매핑을 정의한다.
5. hard veto와 개인별 만족도 계산 노드를 추가한다.
6. Ranker가 LLM의 임의 순서가 아니라 계산된 점수를 따르도록 바꾼다.
7. 계산된 순서를 바꾸지 않는 범위에서 추천 사유를 생성한다.
8. 노드 실행 문서와 테스트를 함께 갱신한다.

### 2.13 구현 후 반드시 확인할 테스트

- 같은 Vocabulary 코드라도 참여자가 다르면 개인별로 따로 계산되는지
- 알레르기와 일치하는 후보가 점수 계산 전에 제외되는지
- `STRONG + NEGATIVE`와 직접 충돌하는 후보가 제외되는지
- 넓은 비선호와 구체적인 긍정 예외에서 specificity 규칙이 적용되는지
- 다른 참여자의 긍정 선호가 한 참여자의 veto를 해제하지 않는지
- 선호가 없는 참여자의 만족도가 중립값 `0.5`인지
- `1.0, 1.0, 0.0` 후보보다 `0.6, 0.6, 0.6` 후보가 먼저 오는지
- 동점 정렬이 매번 동일한지
- 최종 요청·응답 JSON이 기존 계약과 동일한지
- LLM이 입력에 없는 후보 ID·사용자 ID·Vocabulary 코드를 반환했을 때 서버가 제거하는지
...
#### 5. 구현 전에는 목표 설계로만 제시해야 한다

이 장은 현재 동작 설명이 아니다. 실제 노드·테스트·trace가 만들어지기 전 발표 자료에서는 반드시 “구현 예정”으로 표시해야 한다.
...
실제 구현 전에는 이 화면을 “목표 계산 예시”라고 표시한다. 구현과 테스트가 끝난 뒤 실제 trace 또는 테스트 실행 결과로 교체한다.

### 2.16 발표용 1분 설명 초안

> 기존 요청에는 이미 참여자별 사용자 ID와 개인 선호가 구분돼 들어옵니다. 하지만 현재 AI 코드에서는 이를 하나의 선호 목록으로 합치기 때문에, 특정 후보가 누구를 만족시키고 누구를 소외시키는지 계산할 수 없습니다. 그래서 요청과 응답 스키마는 그대로 두고, AI 내부에서 참여자 구분을 랭킹 단계까지 보존하는 방식으로 설계했습니다. 먼저 알레르기와 강한 비선호에 충돌하는 후보는 다른 사람들의 높은 점수로 상쇄되지 않도록 강제 제외합니다. 나머지 후보는 참여자별 만족도를 0에서 1 사이로 계산하고, 전체 평균 만족도 70%와 가장 낮은 개인 만족도 30%를 결합합니다. 예를 들어 두 사람은 완전히 만족하지만 한 사람은 전혀 만족하지 못하는 후보보다, 세 사람 모두가 적당히 만족하는 후보가 더 높은 점수를 받습니다. 의미 관계 판단은 LLM이 담당하지만, veto와 개인별 집계, 평균, 최솟값, 최종 정렬은 서버 코드가 고정된 수식으로 수행합니다. 다만 이 부분은 현재 설계 단계이며, 실제 구현과 테스트가 끝난 뒤에만 구현 결과로 발표할 수 있습니다.
...
아니다. 알레르기와 충돌하는 후보는 점수 계산 전에 제외한다. 안전 조건은 다른 사람들의 높은 만족도로 상쇄할 수 있는 선호가 아니라고 판단했다.

**Q. Back 수정 없이 정말 가능한가요?**

가능하다. 현재 요청에 `participants[].userId`와 각 참여자의 `preferences[]`가 이미 들어온다. AI 내부에서 이 구조를 평탄화하지 않고 보존하면 된다. 점수도 응답에 추가하지 않고 기존 `rank`, `reasons`, `tags`, `summary`에만 반영한다.
...
### 2.18 현재 코드 근거와 구현 예정 지점

현재 동작 근거:

- 참여자별 요청 스키마: `app/schemas/candidates.py`
- 참여자 선호를 평탄화하는 현재 라우트: `app/api/routes/meetings.py`
- 평탄화된 선호를 보관하는 현재 상태: `app/graph/state.py`
- LLM이 후보 순서를 직접 결정하는 현재 Ranker: `app/graph/nodes/n_candidate_ranker.py`
- 현재 랭킹 프롬프트: `app/prompts/n_candidate_ranker.py`
- Back↔AI 요청·응답 계약: `docs/api-design2-backend.md`

구현 시에는 공정성 계산 노드와 테스트를 추가하고, 실제 노드 흐름이 달라지므로 `docs/ai-pipeline-walkthrough.md`도 함께 갱신해야 한다.
