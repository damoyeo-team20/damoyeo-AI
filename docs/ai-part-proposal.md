...
2. **`strength` 타입 불일치**: `ai-pipeline-design.md`(N1 설계)는 `low/medium/high` 3단계를 권고했지만, 실제 백엔드 계약(6장)은 `0.0~1.0` 연속값이다. 이 문서는 계약을 최종으로 채택했다 — LLM이 연속값을 얼마나 일관되게 뽑는지는 구현하면서 검증 필요.
