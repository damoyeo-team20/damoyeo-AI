...
from app.graph.nodes import n2_context_parser, n2_context_router, n2_date_reselector
...
def test_graph_routes_to_generate_reply_without_candidate_dates(monkeypatch):
    ...
    monkeypatch.setattr(n2_context_parser, "get_llm", lambda: _ChatLLM())

    def _fail_router(*_args, **_kwargs):
        raise AssertionError("candidate_dates가 없으면 라우터 LLM을 호출하면 안 된다")

    monkeypatch.setattr(n2_context_router, "get_llm", _fail_router)
...
def test_graph_routes_to_reselect_date_when_intent_detected(monkeypatch):
    ...
    monkeypatch.setattr(
        n2_context_router, "get_llm", lambda: SimpleNamespace(with_structured_output=lambda _s: _RouterLLM())
    )
    monkeypatch.setattr(
        n2_date_reselector,
        "get_llm",
    ...
    monkeypatch.setattr(n2_context_parser, "get_llm", _fail_chat)
