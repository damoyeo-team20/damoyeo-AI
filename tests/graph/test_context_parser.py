...
from app.graph.nodes import n_context_parser
...
def test_generate_context_reply_sends_history_and_new_message(monkeypatch):
    llm = _ChatLLM()
    monkeypatch.setattr(n_context_parser, "get_llm", lambda: llm)
    ...
    response = asyncio.run(
        n_context_parser.generate_context_reply(history, "너무 시끄러운 곳은 피하고 싶어요")
    )
...
def test_finalize_meeting_context_summarizes_full_history(monkeypatch):
    ...
    monkeypatch.setattr(n_context_parser, "get_llm", lambda: _LLM())
    ...
    response = asyncio.run(n_context_parser.finalize_meeting_context(history))
