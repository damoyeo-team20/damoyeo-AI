...
from app.graph.nodes import n2_date_reselector
...
def test_reselect_date_moves_selected_flag_when_date_is_clear(monkeypatch):
    ...
    monkeypatch.setattr(n2_date_reselector, "get_llm", lambda: _LLM())

    result = asyncio.run(
        n2_date_reselector.reselect_date(
...
def test_build_reselect_schema_rejects_dates_outside_candidates():
    schema = n2_date_reselector._build_reselect_schema(_CANDIDATES)
